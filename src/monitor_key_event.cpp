...
#define KEY_LONG_PERIOD         800

#define KEY_CONTINUE_PERIOD     400

#define KEY_TIME_WAIT   600
#define TRUE    1
#define FALSE   0
...
bool continue_pressd_flag = false;
unsigned long cur_ms = 0;
unsigned long fir_ms = 0;
...
void keyScan(unsigned long cur_ms,struct input_event event, KEY_HANDLE_T *key)
{
    if(key == NULL)
    {
    ...
    switch(key->eKeyCurState)
    {
        case APP_KEY_STATE_INIT:
            ...
            else
            {
                ...
                //按键计数加1
                key_press_count++;
                if ( key_press_count > 1 )//二次进入,判断与上次是否键值一样
                {
                    if(KeepValue != key->eKeyId){
                        KeepValue = key->eKeyId;
                        key_press_count = 1;
                    }
                }else{//首次进入记住按键
                    KeepValue = key->eKeyId;
        ...
        case APP_KEY_STATE_LONG:
            ...
            else
            {
                ...
                //
                key_press_count++;
                if ( key_press_count > 1 )//首次进入,仅仅记录首次
                {
                    if(KeepValue != key->eKeyId){
                        KeepValue = key->eKeyId;
                        key_press_count = 1;
...
const char * MonitorKeyEvent::get_label(const struct label *labels, int value)
{
...
int MonitorKeyEvent::print_input_props(int fd)
{
...
int MonitorKeyEvent::print_possible_events(int fd, int print_flags)
{
    uint8_t *bits = NULL;
    ssize_t bits_size = 0;
    const char* label;
...
void MonitorKeyEvent::print_hid_descriptor(int bus, int vendor, int product)
{
...
int MonitorKeyEvent::open_device(const char *device, int print_flags)
{
    int version;
    int fd;
...
int MonitorKeyEvent::close_device(const char *device, int print_flags)
{
    int i;
    for(i = 1; i < nfds; i++) {
...
int MonitorKeyEvent::read_notify(const char *dirname, int nfd, int print_flags)
{
    int res;
...
int MonitorKeyEvent::scan_dir(const char *dirname, int print_flags)
{
    char devname[PATH_MAX];
    char *filename;
    DIR *dir;
...
void MonitorKeyEvent::usage(char *name)
{
...
MonitorKeyEvent::MonitorKeyEvent(int argc, char *argv[])
{
    //*************************************************
    ...
    do{
        ...
        if(pollres == 0 || send_flag)
        {
            if(key_press_count==0)
                ...
                else if(KeepValue == APP_KEY2)
                    message<<QString("%1:%2").arg(KEY_VOLUMEUP).arg(key_press_count);
                else if(KeepValue == APP_KEY3)
                    message<<QString("%1:%2").arg(KEY_PLAYPAUSE).arg(key_press_count);
                QDBusConnection::systemBus().send(message);
                printf("\n********key_press_count:%d,keyValue:%d********\n",key_press_count,KeepValue);
            }
            send_flag = false;
            fir_ms = 0;
            key_press_count = 0;
...
MonitorKeyEvent::~MonitorKeyEvent()
{
